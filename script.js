...
const RSVP_CONFIG = {
  whatsappNumber: "+91 9959837921",
  googleFormAction: "https://docs.google.com/forms/d/e/1FAIpQLSdrn3d71nvEGYLOc5aWIsFSyK6jamqOaOTpWtXJJCUCvclcOA/formResponse",
  googleFormFields: {
    name: "entry.1557807799",
    email: "entry.61740546",
    phone: "entry.897305310",
    guests: "entry.641086561",
    attending: "entry.1353064031",
    message: "entry.194300241",
  },
};
...
async function submitToGoogleForm(payload) {
  const formAction = RSVP_CONFIG.googleFormAction.trim();
  if (!formAction) {
    return false;
  }

  const formPayload = new URLSearchParams();
  const fieldMap = RSVP_CONFIG.googleFormFields;

  Object.entries(fieldMap).forEach(([key, entryId]) => {
    if (!entryId || !payload[key]) {
      return;
    }

    formPayload.set(entryId, payload[key]);
  });
